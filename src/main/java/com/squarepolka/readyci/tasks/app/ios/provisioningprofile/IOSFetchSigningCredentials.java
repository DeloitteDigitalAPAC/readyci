...
public class IOSFetchSigningCredentials extends Task {
    ...
    public void performTask(BuildEnvironment buildEnvironment) throws TaskFailedException {
        ...
        try {
            // copy the provisioning profile and certificate files into the project path
            executeCommand(new String[]{"tar", "-xvf", credentialsTarLocation, "-C", buildEnvironment.getProjectPath(), "*.mobileprovision", "*.p12"});
        }
        catch (Exception e) {
